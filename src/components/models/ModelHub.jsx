...
export default function ModelHub() {
  ...
  useEffect(() => {
    console.log('Environment Variables:', {
      ...
      NEXT_PUBLIC_LITELLM_API_URL: process.env.NEXT_PUBLIC_LITELLM_API_URL
    });
  }, []);
  ...
  const fetchAPIKey = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/key/info`,
        {
          method: 'GET',
          headers: {
  ...
  }, [userDetails.lite_llm_key, fetchModels]);
          ...
          {/* {!isLoadingUser && (
            <div className="flex items-center mt-4 sm:mt-0">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{userDetails.email_address}</p>
              </div>
            </div>
          )} */}
        </div>
          ...
          <div className="mb-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
              <input
                ...
                value={isLoadingKey ? 'Loading...' : userDetails.lite_llm_key || '••••••••••••••••••••••••••••••••••'}
                readOnly
                className={`flex-grow p-2 rounded border ${borderColor} ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} text-sm`}
              />
        ...
        <div className={`mb-6 flex flex-col sm:flex-row gap-3`}>
          <div className="relative flex-grow">
            <input
              ...
              className={`w-full p-2 pl-10 rounded-lg border ${borderColor} ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} text-sm`}
            />
          ...
          <select
            ...
            className={`p-2 rounded-lg border ${borderColor} ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} text-sm`}
          >
          ...
          {filteredModels.map(model => (
            <div key={model.id} className={`rounded-lg ${cardBg} border ${borderColor} overflow-hidden shadow-sm hover:shadow-md transition`}>
              <div className="p-4 sm:p-6">
                <div className="flex items-center justify-center w-12 h-12 mb-4 bg-white rounded-lg p-2">
                  <img 
                    src={model.icon} 
                    alt={model.name} 
